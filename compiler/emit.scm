...
(define (emit-label-defines labels f)
  (define label-num 2)
  (for-each (lambda (l)
              (file-append "#define " f)
              (file-append (symbol->string l) f)
              (file-append  " " f)              
              (file-append (number->string label-num) f)
              (file-append "\n" f)
...
(define (emit-constant const filename)
  (cond ((number? const)
         ...
         (file-append ")" filename))
        ((null? const)
         (file-append "NIL" filename))
        ((symbol? const)
         (file-append "obj_new_symbol(\"" filename)
         (file-append (symbol->string const) filename)
         (file-append "\")" filename))
        (error "ERROR constant not a number or null")))

(define (emit-case label filename)
  (file-append "case " filename)
  (file-append (symbol->string label) filename)
  (file-append ":;\n" filename))

(define (emit-label label filename)
  (file-append (symbol->string label) filename))
...
(define (emit-statement statement filename)
  (define (emit-r parts)
    (cond ((null? parts) 'done)
          ...
          ((symbol? (car parts)) (emit-case (car parts) filename))
          ((cons? (car parts))
           (cond ((eq? 'reg (caar parts))
                 ...
                 ((eq? 'label (caar parts))
                  (emit-label (cadar parts) filename))
                 (else ; expression
                  (emit-r (car parts)))))
          (else 'ERROR))
    (emit-r (cdr parts)))
